...
mesh_t *read_mesh_from_file(char *filename, int has_texture) {
  ...
  if (file == NULL) {
    printf(
        "Error: Unable to open file %s. Is the file in the correct folder?\n",
        filename);
    return NULL;
  }
  ...
  while (fgets(line, sizeof(line), file)) {
    if (line[0] == 'v') {
      if (line[1] == 't') {
        ...
        textures =
            (vec2d_t *)realloc(textures, (texture_count + 1) * sizeof(vec2d_t));
        ...
        vertices =
            (vec3d_t *)realloc(vertices, (vertex_count + 1) * sizeof(vec3d_t));
    ...
    if (!has_texture) {
      if (line[0] == 'f' && line[1] == ' ') {
        ...
        triangle_t face;
        face = (triangle_t){vertices[verts[0] - 1], vertices[verts[1] - 1],
                            vertices[verts[2] - 1]};

        faces =
            (triangle_t *)realloc(faces, (face_count + 1) * sizeof(triangle_t));
        faces[face_count++] = face;
      }
    } else {
      if (line[0] == 'f') {
        int tokens[6];
        sscanf(line, "f %d/%d %d/%d %d/%d", &tokens[0], &tokens[1], &tokens[2],
               &tokens[3], &tokens[4], &tokens[5]);

        triangle_t face;
        face = (triangle_t){vertices[tokens[0] - 1], vertices[tokens[2] - 1],
                            vertices[tokens[4] - 1], textures[tokens[1] - 1],
                            textures[tokens[3] - 1], textures[tokens[5] - 1]};
...
int triangle_clip_against_plane(vec3d_t *plane_point, vec3d_t *plane_normal,
                                triangle_t *triangle_in,
                                triangle_t *triangle_out_1,
                                triangle_t *triangle_out_2) {
  *plane_normal = vec3_normal(plane_normal);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    distance[i] = vec3_dot(
        plane_normal, &v); // plane_point) - (plane_normal->x * v.x
                           // +plane_normal->y * v.y + plane_normal->z * v.z);
  }
  ...
  if (n_inside_point_count == 1 && n_outside_point_count == 2) {
    ...
    triangle_out_1->verts[1] = vec3_intersect_plane(
        plane_point, plane_normal, inside_points[0], outside_points[0], &t);

    triangle_out_1->texture[1].u =
        t * (outside_texture[0]->u - inside_texture[0]->u) +
        inside_texture[0]->u;
    triangle_out_1->texture[1].v =
        t * (outside_texture[0]->v - inside_texture[0]->v) +
        inside_texture[0]->v;
    triangle_out_1->texture[1].w =
        t * (outside_texture[0]->w - inside_texture[0]->w) +
        inside_texture[0]->w;

    triangle_out_1->verts[2] = vec3_intersect_plane(
        plane_point, plane_normal, inside_points[0], outside_points[1], &t);

    triangle_out_1->texture[2].u =
        t * (outside_texture[1]->u - inside_texture[0]->u) +
        inside_texture[0]->u;
    triangle_out_1->texture[2].v =
        t * (outside_texture[1]->v - inside_texture[0]->v) +
        inside_texture[0]->v;
    triangle_out_1->texture[2].w =
        t * (outside_texture[1]->w - inside_texture[0]->w) +
        inside_texture[0]->w;
  ...
  if (n_inside_point_count == 2 && n_outside_point_count == 1) {
    ...
    triangle_out_2->verts[2] = vec3_intersect_plane(
        plane_point, plane_normal, inside_points[1], outside_points[0], &t);

    triangle_out_2->texture[2].u =
        t * (outside_texture[0]->u - inside_texture[1]->u) +
        inside_texture[1]->u;
    triangle_out_2->texture[2].v =
        t * (outside_texture[0]->v - inside_texture[1]->v) +
        inside_texture[1]->v;
    triangle_out_2->texture[2].w =
        t * (outside_texture[0]->w - inside_texture[1]->w) +
        inside_texture[1]->w;
